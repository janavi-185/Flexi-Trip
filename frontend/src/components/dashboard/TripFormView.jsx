...
export default function TripFormView({ preferences }) {
  if (!preferences) return <EmptyState />;

  const { destination, budget, duration, travelers, tripStyle, lastMessage } = preferences;
  const badgeClass = STYLE_BADGE_COLORS[tripStyle] || 'bg-secondary text-secondary-foreground';
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        ...
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-primary/10 border border-primary/20">
          ...
          <p className="text-sm text-primary font-medium">Itinerary generated successfully</p>
        </div>
        ...
        {lastMessage && (
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            <div className="px-5 py-3 bg-secondary/50 border-b border-border">
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Your Request</h3>
            </div>
            <div className="px-5 py-4">
              <p className="text-sm text-foreground leading-relaxed italic">"{lastMessage}"</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { emoji: '✅', label: 'Status', value: 'Planned' },
            { emoji: '🗓️', label: 'Created', value: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) },
          ].map((stat) => (
